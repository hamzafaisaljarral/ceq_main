...
from ceq_user.database.models import User, Category, Companies
...
class NonComplianceContributerVistor(Resource):
    def get(self):
        ...
        for company in companies:
            company_dict = company.to_mongo().to_dict()
            company_dict['_id'] = str(company_dict['_id'])  # Convert ObjectId to string
...
class BusinessComplianceandNonComplianceImages(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            return jsonify({'error': 'Invalid date format'}), 400
        result = get_images_with_compliance(start_date,end_date, region)
